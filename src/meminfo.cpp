...
void MemoryInfo::updateMemoryStats()
{
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		// Calculate usage as a 0.0-1.0 fraction, matching CpuInfo::history
		// so both graphs can share LineGraph's maxValue: 1.0 convention.
		double usageFraction = 0.0;
		if (m_total > 0) {
			usageFraction = static_cast<double>(m_inUse) / static_cast<double>(m_total);
		}

		// Update history for the graph
		m_usageHistory.removeFirst();
		m_usageHistory.append(usageFraction);
...
QString MemoryInfo::slotsInUse() const { return m_slots; }
